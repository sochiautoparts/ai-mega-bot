"""Chutes AI Provider — unlimited free API, OpenAI-compatible.

Chutes AI provides unlimited free access to 100+ open-source models.
No rate limits, no token caps, no credit card required.
Models: DeepSeek-V3, Qwen3 235B, Llama 4, and more.
...
class ChutesProvider(BaseProvider):
    """Chutes AI provider — unlimited free, OpenAI-compatible API."""

    name: str = "chutes"
    supports_streaming: bool = False

    # Chutes can work with or without API key (key gives higher rate limits)
    ...
    def is_available(self) -> bool:
        """Chutes is always available — works with or without API key."""
        return True
    ...
    async def generate(self, prompt: str, **kwargs) -> AIResponse:
        """Generate text via Chutes AI chat completions with context."""
        if not self._client:
            await self.init()
    ...
    async def translate(
        ...
        **kwargs,
        ...
        """Translate text using Chutes AI with translation system prompt."""
        lang_names = {
